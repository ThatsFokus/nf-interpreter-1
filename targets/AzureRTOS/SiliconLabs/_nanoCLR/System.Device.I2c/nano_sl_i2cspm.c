...
// Portions Copyright 2020 Silicon Laboratories Inc. www.silabs.com
...
#define I2CSPM_TRANSFER_TIMEOUT 300
#endif
...
void I2CSPM_Init(I2CSPM_Init_TypeDef *init)
{
    ...
    init->port->ROUTELOC0 =
        (uint32_t)((init->portLocationSda << _I2C_ROUTELOC0_SDALOC_SHIFT) | (init->portLocationScl << _I2C_ROUTELOC0_SCLLOC_SHIFT));
#elif defined(_I2C_ROUTE_MASK)
    init->port->ROUTE = I2C_ROUTE_SDAPEN | I2C_ROUTE_SCLPEN | (init->portLocation << _I2C_ROUTE_LOCATION_SHIFT);
#else
#if defined(I2C0)
    if (init->port == I2C0)
    {
        ...
        GPIO->I2CROUTE[0].SCLROUTE =
            (uint32_t)((init->sclPin << _GPIO_I2C_SCLROUTE_PIN_SHIFT) | (init->sclPort << _GPIO_I2C_SCLROUTE_PORT_SHIFT));
        GPIO->I2CROUTE[0].SDAROUTE =
            (uint32_t)((init->sdaPin << _GPIO_I2C_SDAROUTE_PIN_SHIFT) | (init->sdaPort << _GPIO_I2C_SDAROUTE_PORT_SHIFT));
    }
#endif
#if defined(I2C1)
    if (init->port == I2C1)
    ...
    if (init->port == I2C2)
    {
        ...
        GPIO->I2CROUTE[2].SCLROUTE =
            (uint32_t)((init->sclPin << _GPIO_I2C_SCLROUTE_PIN_SHIFT) | (init->sclPort << _GPIO_I2C_SCLROUTE_PORT_SHIFT));
        GPIO->I2CROUTE[2].SDAROUTE =
            (uint32_t)((init->sdaPin << _GPIO_I2C_SDAROUTE_PIN_SHIFT) | (init->sdaPort << _GPIO_I2C_SDAROUTE_PORT_SHIFT));
    }
#endif
#endif
