...
// This is an extension to the original file from Gecko SDK v4.1.1
// Following Support Case #00293097 with Silabs, we where advised to add the missing
// APIs to our own implementation. In case these are ever made available in the
// official SDK, this file should be removed and the original one should be used.
// There is also a change to allow changing the USB Vendor Class description, by replacing
// the hardcoded "Vendor-specific class" with a custom one. Suggestion to improve this has
// been made to Silabs (Support Case #00292362).
                                                ...
                                                uint8_t config_nbr)
{
  ...
  // .NET nanoFramework change: replace hardcoded USB Vendor Class description with one from the configuration
...
sl_status_t sl_usbd_vendor_abort_read_bulk(uint8_t class_nbr)
{
    ...
    // Check if a xfer is already in progress.
    if (p_ctrl->comm_ptr->data_bulk_out_active_transfer)
    {
...
sl_status_t sl_usbd_vendor_abort_write_bulk(uint8_t class_nbr)
{
    ...
    // Check if another xfer is already in progress.
    if (p_ctrl->comm_ptr->data_bulk_in_active_transfer)
    {
