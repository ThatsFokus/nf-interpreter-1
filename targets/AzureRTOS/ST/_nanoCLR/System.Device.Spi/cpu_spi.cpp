...
uint16_t ComputeBaudRate(SPI_DEVICE_CONFIGURATION &config, int32_t &actualFrequency)
{
    ...
    // Could not find STM32_SPI1_MAX for STM32L4xx.
    // Theoretical fastest SPI clock is STM32_PCLK2/2
    ...
    if (busIndex == 2 || busIndex == 3)
    {
        ...
        // Could not find STM32_SPI23_MAX for STM32L4xx.
        // Theoretical fastest SPI clock is STM32_PCLK1/2
        maxSpiFrequency = STM32_PCLK1 / 2;
    }
...
#error "Error setting max SPI frequency. Check if the target series is defined."
    ...
    {
        // Note the SPI clock is minimum div 2 when divider == 0;
