 ...
 * @{
 */
#define TIME_IMMEDIATE ((sysinterval_t)TX_NO_WAIT)
#define TIME_INFINITE  ((sysinterval_t)TX_WAIT_FOREVER)
/** @} */
 ...
 */
#define OSAL_ST_FREQUENCY TX_TIMER_TICKS_PER_SECOND
...
#define OSAL_MS2I(msecs)                                                                                               \
    ((sysinterval_t)((((time_conv_t)(msecs) * (time_conv_t)OSAL_ST_FREQUENCY) + (time_conv_t)999) / (time_conv_t)1000))
...
#define OSAL_US2I(usecs)                                                                                               \
    ((sysinterval_t)((((time_conv_t)(usecs) * (time_conv_t)OSAL_ST_FREQUENCY) + (time_conv_t)999999) / (time_conv_t)1000000))
...
#define OSAL_I2S(interval)                                                                                             \
    (time_secs_t)(                                                                                                     \
        ((time_conv_t)(interval) + (time_conv_t)OSAL_ST_FREQUENCY - (time_conv_t)1) / (time_conv_t)OSAL_ST_FREQUENCY)
...
#define OSAL_I2MS(interval)                                                                                            \
    (time_msecs_t)(                                                                                                    \
        (((time_conv_t)(interval) * (time_conv_t)1000) + (time_conv_t)OSAL_ST_FREQUENCY - (time_conv_t)1) /            \
...
#define OSAL_I2US(interval)                                                                                            \
    (time_msecs_t)(                                                                                                    \
        (((time_conv_t)(interval) * (time_conv_t)1000000) + (time_conv_t)OSAL_ST_FREQUENCY - (time_conv_t)1) /         \
        (time_conv_t)OSAL_ST_FREQUENCY)
/** @} */
...
{
    interrupt_save = __disable_interrupts();
}
...
{
    TX_DISABLE
    return (syssts_t)interrupt_save;
}
...
{
    return (bool)((systime_t)((systime_t)time - (systime_t)start) < (systime_t)((systime_t)end - (systime_t)start));
}
...
static inline void osalThreadQueueObjectInit(threads_queue_t *tqp)
{
    ...
    tqp->prev = (queue_elem_t *)tqp;
    tqp->next = (queue_elem_t *)tqp;
}
...
static inline void osalMutexObjectInit(mutex_t *mp)
{
    ...
    tx_mutex_create(mp, NULL, TX_NO_INHERIT);
}
