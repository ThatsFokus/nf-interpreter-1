...
class Timer
{
  public:
    Timer(HANDLE hQueue, UINT32 initialPeriod, UINT32 period, TimerOption flags, std::function<void()> func)
        : hQueue(hQueue), hTimer(NULL), Callback(std::make_unique<std::function<void()>>(func)), Period(period),
          Flags(flags | (period == 0 ? TimerOption::OneShot : TimerOption::ExecuteDefault))
    {
        Start(initialPeriod, period);
    }

    Timer() : Timer(nullptr, 0, 0, TimerOption::ExecuteDefault, []() {})
    {
    }
    ...
    Timer(TimerOption flags, std::function<void()> func) : Timer(nullptr, 0, 0, flags, func)
    {
    }

    Timer(std::function<void()> func) : Timer(nullptr, 0, 0, TimerOption::ExecuteDefault, func)
    {
    }
