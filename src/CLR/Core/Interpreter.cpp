...
#define RESETSTACK()                                                                                                   \
    {                                                                                                                  \
        g_CLR_RT_fBadStack = false;                                                                                    \
    }
#define CHECKSTACK(stack, evalPos)                                                                                     \
    {                                                                                                                  \
        g_CLR_RT_fBadStack = stack->m_evalStackPos <= evalPos;                                                         \
    }
...
//--//
#define EMPTYSTACK(stack, evalPos)                                                                                     \
    {                                                                                                                  \
        evalPos = &stack->m_evalStack[-1];                                                                             \
        RESETSTACK();                                                                                                  \
    }
#define READCACHE(stack, evalPos, ip, fDirty)                                                                          \
    {                                                                                                                  \
...
#define WRITEBACK(stack, evalPos, ip, fDirty)                                                                          \
    {                                                                                                                  \
        stack->m_evalStackPos = &evalPos[+1];                                                                          \
        stack->m_IP = ip;                                                                                              \
        fDirty = false;                                                                                                \
...
HRESULT CLR_RT_HeapBlock::Convert_Internal(CLR_DataType et)
{
    ...
    switch (dt)
    {
        ...
        case DATATYPE_U4:
        {
            ...
            if ((dtlSrc.m_flags & CLR_RT_DataTypeLookup::c_Signed) &&
                (dtlDst.m_flags & CLR_RT_DataTypeLookup::c_Signed))
            {
    ...
    if (scaleOut != scaleIn)
    {
        ...
        {
            case -1:
                val = (double)((CLR_UINT64_TEMP_CAST)NumericByRef().u8);
                break;
            case 0:
                val = (double)((CLR_INT64_TEMP_CAST)NumericByRef().s8);
                break;
            case 1:
                val = (double)NumericByRef().r4;
                break;
            case 2:
                val = NumericByRef().r8;
                break;
        ...
        switch (scaleOut)
        {
            ...
            case -1:
                NumericByRef().u8 = (CLR_UINT64)(CLR_INT64)val;
                break;
            case 0:
                NumericByRef().s8 = (CLR_INT64)val;
                break;
            case 1:
                NumericByRef().r4 = (float)val;
                break;
            case 2:
                NumericByRef().r8 = val;
                break;
...
CLR_RT_Thread::UnwindStack *CLR_RT_Thread::PushEH()
{
    ...
    else
    {
        ...
        memmove(
            &m_nestedExceptions[0],
            &m_nestedExceptions[1],
            sizeof(m_nestedExceptions) - sizeof(m_nestedExceptions[0]));
...
void CLR_RT_Thread::PopEH_Inner(CLR_RT_StackFrame *stack, CLR_PMETADATA ip)
{
    ...
    if (m_nestedExceptionsPos > 0)
    {
        ...
        //
        if (m_nestedExceptions[m_nestedExceptionsPos - 1].m_stack != stack)
            return;
    ...
    if (stack->m_call.m_target->flags & CLR_RECORD_METHODDEF::MD_HasExceptionHandlers)
    {
            ...
            case CLR_RT_StackFrame::c_MethodKind_Interpreted:
            {
                ...
                if (assm->FindMethodBoundaries(stack->m_call.Method(), ipMethod_Start, ipMethod_End))
                {
                    ...
                    CLR_RECORD_EH::ExtractEhFromByteCode(
                        stack->m_IPstart + (ipMethod_End - ipMethod_Start),
                        ptrEh2,
                        numEh2);
        ...
        while (numEh--)
        {
            ...
            if (s_CLR_RT_fTrace_Exceptions >= c_CLR_RT_Trace_Annoying)
            {
                if (to == NULL || s_CLR_RT_fTrace_Exceptions >= c_CLR_RT_Trace_Obnoxious)
                {
                    CLR_Debug::Printf(
                        ...
                        (size_t)ptrEhExt->m_tryStart,
                        (size_t)ptrEhExt->m_tryEnd,
                        ptrEhExt->m_handlerStart - stack->m_IPstart);
                }
            }
#endif
            ...
            if (ptrEhExt->IsFilter() && ptrEhExt->m_userFilterStart <= from && from < ptrEhExt->m_handlerStart)
            {
                ...
                // Therefore, reset the IP to inside the original try block so we can try the next handler in this
                // sequence:
                // Try
                //     ...            //2. set IP to here
                // Catch When False
...
HRESULT CLR_RT_Thread::Execute_Inner()
{
    ...
    while (m_timeQuantumExpired == false && !CLR_EE_DBG_IS(Stopped))
    {
        ...
        if (stack->m_flags & CLR_RT_StackFrame::c_ProcessSynchronize)
        {
            ...
            //
            if (stack->m_flags &
                (CLR_RT_StackFrame::c_PendingSynchronizeGlobally | CLR_RT_StackFrame::c_PendingSynchronize))
            {
...
#define OPDEF(name, string, pop, push, oprType, opcType, l, s1, s2, ctrl) case name:
                ...
                // Stack: ... <arg1> <arg2> ... <argN> -> ...
                {
                    ...
                    else
                    {
                        ...
                        //
                        // We have to insert the 'this' pointer as argument 0, that means moving all the arguments up
                        // one slot...
                        //
                        top--;
                        while (--changes > 0)
                        ...
                        // But the constructor will try to load the 'this' pointer and since it's a value type, it will
                        // be cloned. For the length of the constructor, change the type from an object pointer to a
                        // reference.
                        //
                        // See CLR_RT_StackFrame::Pop()
                        //
                ...
                OPDEF(CEE_NEWARR, "newarr", PopI, PushRef, InlineType, IObjModel, 1, 0xFF, 0x8D, NEXT)
                {
                    ...
                    stack->m_flags &=
                        ~CLR_RT_StackFrame::c_CompactAndRestartOnOutOfMemory; // we do not need this in this case
                ...
                // Stack: ... <obj> <index> -> <address> ...
                {
                    ...
                    NANOCLR_CHECK_HRESULT(
                        evalPos[0].InitializeArrayReference(evalPos[0], evalPos[1].NumericByRef().s4));
                ...
                // Stack: ... <obj> <index> -> <value> ...
                {
                    ...
                    //
                    // To load an element from an array, we first initialize a temporary reference to the element and
                    // then dereference it.
                    //
                ...
                OPDEF(CEE_CGT_UN, "cgt.un", Pop1 + Pop1, PushI, InlineNone, IPrimitive, 2, 0xFE, 0x03, NEXT)
                {
                    ...
                    evalPos[0].SetInteger(
                        CLR_RT_HeapBlock::Compare_Unsigned_Values(evalPos[0], evalPos[1]) > 0 ? 1 : 0);
                    break;
                }
...
#undef OPDEF
...

        {
            ...
            switch (res.DataType())
            {
                ...
                case DATATYPE_BOOLEAN:
                    res.ChangeDataType(DATATYPE_I4);
                    res.NumericByRef().u4 = (CLR_UINT32)res.NumericByRef().u1;
                    break;
                case DATATYPE_I1:
                    res.ChangeDataType(DATATYPE_I4);
                    res.NumericByRef().s4 = (CLR_INT32)res.NumericByRef().s1;
                    break;
                case DATATYPE_U1:
                    res.ChangeDataType(DATATYPE_I4);
                    res.NumericByRef().u4 = (CLR_UINT32)res.NumericByRef().u1;
                ...
                case DATATYPE_CHAR:
                    res.ChangeDataType(DATATYPE_I4);
                    res.NumericByRef().u4 = (CLR_UINT32)res.NumericByRef().u2;
                    break;
                case DATATYPE_I2:
                    res.ChangeDataType(DATATYPE_I4);
                    res.NumericByRef().s4 = (CLR_INT32)res.NumericByRef().s2;
                    break;
                case DATATYPE_U2:
                    res.ChangeDataType(DATATYPE_I4);
                    res.NumericByRef().u4 = (CLR_UINT32)res.NumericByRef().u2;
