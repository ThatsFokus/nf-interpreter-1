...
HRESULT Library_corlib_native_System_Threading_Thread::_ctor___VOID__SystemThreadingThreadStart(
...
HRESULT Library_corlib_native_System_Threading_Thread::Start___VOID(CLR_RT_StackFrame &stack)
{
    ...
    pThis = stack.This();
    FAULT_ON_NULL(pThis);
    dlg = pThis[FIELD___delegate].DereferenceDelegate();
    FAULT_ON_NULL(dlg);
...
HRESULT Library_corlib_native_System_Threading_Thread::get_Priority___SystemThreadingThreadPriority(
    CLR_RT_StackFrame &stack)
{
    NATIVE_PROFILE_CLR_CORE();
    NANOCLR_HEADER();
    ...
    if (pri < ThreadPriority::Lowest)
        pri = ThreadPriority::Lowest;
    else if (pri > ThreadPriority::Highest)
        pri = ThreadPriority::Highest;
...
HRESULT Library_corlib_native_System_Threading_Thread::set_Priority___VOID__SystemThreadingThreadPriority(
    CLR_RT_StackFrame &stack)
{
    NATIVE_PROFILE_CLR_CORE();
    NANOCLR_HEADER();
    ...
    if (stack.m_customState == 0)
    {
        ...
        // Get argument for priority. Check the range - should be between ThreadPriority::Highest and
        // ThreadPriority::Lowest ( 0 - 4 )
...
HRESULT Library_corlib_native_System_Threading_Thread::Join___BOOLEAN__SystemTimeSpan(CLR_RT_StackFrame &stack)
{
    ...
    timeExpireIn = Library_corlib_native_System_TimeSpan::GetValuePtr(stack.Arg1());
    FAULT_ON_NULL(timeExpireIn);
...
HRESULT Library_corlib_native_System_Threading_Thread::get_ThreadState___SystemThreadingThreadState(
    CLR_RT_StackFrame &stack)
{
    NATIVE_PROFILE_CLR_CORE();
        ...
        case 0:
            ...
            if (timeSleep != 0)
            {
                NANOCLR_CHECK_HRESULT(
                    g_CLR_RT_ExecutionEngine.InitTimeout(timeExpire, stack.m_arguments[0].NumericByRef().s4));
    ...
    CLR_RT_StackFrame &stack)
{
...
#if defined(NANOCLR_APPDOMAINS)
        {
            CLR_RT_ObjectToEvent_Source *appDomainSrc =
                CLR_RT_ObjectToEvent_Source::ExtractInstance(pManagedThread[FIELD___appDomain]);
    ...
    // contains the priority as well as the delegate to start.  However, that state is really just used as a placeholder
...
HRESULT Library_corlib_native_System_Threading_Thread::GetThread(
    CLR_RT_StackFrame &stack,
    CLR_RT_Thread *&th,
    bool mustBeStarted,
    bool noSystemThreads)
{
    NATIVE_PROFILE_CLR_CORE();
    NANOCLR_HEADER();
    CLR_RT_ObjectToEvent_Source *src = GetThreadReference(stack);
